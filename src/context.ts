...
// the coding session.
export const PROXY_MARKER = String.fromCharCode(0x200b, 0x200b, 0x200b, 0x200b);
...
export const PAYMENT_SUCCESS_LINE = '💳 Payment successful — generating your response...';
...
function isMarkedProxyMessage(msg: any): boolean {
  return !!(msg && msg.info && msg.info.role === 'assistant' && textOf(msg).indexOf(PROXY_MARKER) !== -1);
}

function isSelectionReply(msg: any): boolean {
  if (!msg || !msg.info || msg.info.role !== 'user') return false;
...
function withoutPaymentSuccessLine(msg: any): any | null {
  const parts = msg && msg.parts;
  if (!Array.isArray(parts)) return null;
  for (let i = 0; i < parts.length; i++) {
    const part = parts[i];
    if (part && part.type === 'text' && typeof part.text === 'string' && part.text.indexOf(PAYMENT_SUCCESS_LINE) !== -1) {
      const cleaned = part.text.split(PAYMENT_SUCCESS_LINE).join('').replace(/\n{3,}/g, '\n\n');
...
export function filterProxyChatter(messages: any[]): any[] {
