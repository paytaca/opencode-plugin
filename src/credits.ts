...
const MIN_INTERVAL_MS = 60_000; // at most one backend poll per minute
...
const bandForSeconds = (s: number): number => {
  if (s <= 0) return 0; // exhausted
  if (s < 120) return 1; // < 2 min
  if (s < 600) return 2; // < 10 min
  if (s < 1800) return 3; // < 30 min
  return 4; // >= 30 min
};
...
export function createCreditsToastWatch(deps: CreditsToastDeps) {
  ...
  const emitBand = (band: number, remaining: number, name: string) => {
    if (band === 0) {
      push(
        '⛔ Paytaca credits exhausted',
        `${name} is out of time. The next request will ask you to buy a plan (or arm auto-refill).`,
        'error',
      );
    } else if (band === 1) {
      push(
        '🛑 Paytaca credits almost gone',
        `${name}: under ${formatShort(120)} left — buy a plan soon or arm auto-refill.`,
        'error',
      );
    } else if (band === 2) {
      push(
        '⏳ Paytaca credits running low',
        `${name}: about ${formatShort(remaining)} remaining.`,
        'warning',
      );
    }
  };
  ...
  const check = async () => {
    ...
    if (!backendUrl || !walletHash) return;
    ...
    if (!isFinite(lastRemaining)) {
      ...
      push(
        '⚡ Paytaca credits added',
        `${name}: ${formatShort(remaining)} remaining now.`,
        'success',
      );
