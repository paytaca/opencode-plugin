...
exports.updateDependencySpecs = updateDependencySpecs;
exports.deleteStaleLockfiles = deleteStaleLockfiles;
exports.cleanStaleCaches = cleanStaleCaches;
exports.runSelfHeal = runSelfHeal;
...
function updateDependencySpecs(exactVersion, opts) {
    ...
    for (const file of candidates) {
        try {
            ...
            if (typeof deps[PACKAGE_NAME] !== 'string' || deps[PACKAGE_NAME] === exactVersion)
                continue;
...
function deleteStaleLockfiles(opts) {
    const home = opts.home || os.homedir();
    const cwd = opts.cwd || process.cwd();
    const removed = [];
    const lockfiles = [
    ...
    for (const file of lockfiles) {
        try {
            if (!fs.existsSync(file))
                continue;
            if (fs.readFileSync(file, 'utf8').indexOf(PACKAGE_NAME) === -1)
                continue;
...
function cleanStaleCaches(targetVersion, opts) {
    const home = opts.home || os.homedir();
    ...
    for (const entry of entries) {
        if (entry.indexOf('opencode-plugin') === -1)
            continue;
...
async function runSelfHeal(opts = {}) {
    if (healRan)
        return;
    healRan = true;
    if (process.env.PAYTACA_SELF_UPDATE === '0')
        return;
    try {
...
//# sourceMappingURL=selfheal.js.map